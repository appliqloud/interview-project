...
def test_count_orders(user_graphql_context):
    query = '''
            query {
                countOrders
            }
...
def test_find_order_by_id(user_graphql_context, existing_order_id):
    query = '''
    ...
    result = schema.execute_sync(query, context_value = user_graphql_context, variable_values = {'id': existing_order_id})
...
def test_find_orders(user_graphql_context):
    query = '''
            query {
                findOrders {
                    id
...
def test_create_order(user_graphql_context, create_order_payload):
    query = '''
    ...
    result = schema.execute_sync(query, context_value = user_graphql_context, variable_values = {'order': create_order_payload})

    assert not result.errors
    assert 'createOrder' in result.data
    assert result.data['createOrder']['__typename'] == 'Order'
    assert result.data['createOrder']['status'] == 'PENDING'
    assert result.data['createOrder']['product']['id'] == create_order_payload['productId']
    assert result.data['createOrder']['quantity'] == create_order_payload['quantity']

def test_cancel_order(user_graphql_context, existing_order_id):
    query = '''
...
def test_mark_order_as_received(user_graphql_context, existing_order_id):
    query = '''
